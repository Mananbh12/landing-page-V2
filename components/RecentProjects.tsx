            ...
            <PinContainer title={link} href={link}>
              <div className="relative flex items-center justify-center sm:w-[570px] w-[80vw] overflow-hidden sm:h-[40vh] h-[30vh] lg:h-[30vh] mb-10">
                <div className="relative w-full h-full overflow-hidden lg:rounded-3xl bg-[#13162d]">
                  {/* Pas d'image de fond pour éviter l'erreur 404 */}
                  {/* Placeholder visuel : fond coloré pour l'instant */}
                </div>
                {/* Image principale : rendue seulement si img n'est pas vide */}
                {img && (
                  <Image
                    src={img}
                    alt={title}
              ...
              <div className="flex items-center justify-between mt-7 mb-3">
                <div className="flex items-center">
                  {iconLists.map((icon, index) =>
                    // Rendu conditionnel pour les icônes non vides
                    icon ? (
                      <div
                        key={`${id}-icon-${index}`}
